...
argparser.add_argument('--no_cache', action='store_false',
                       help='Whether to cache the html of all people  (Just remove the cache files at the end of the script)')
argparser.add_argument('--no_update', action='store_false',
                       help='Whether to check for updates')
...
if not doCache:
    ...
    for file in os.listdir(DIR_CACHE_ROOT):
        os.remove(os.path.join(DIR_CACHE_ROOT, file))
    print("[INFO] Removed all cached files under the .cache folder.")